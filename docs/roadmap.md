...
- **E06 — Autonomous rule-discovery loop.** A search (Bayesian optimization / genetic programming) that proposes new subrules *and* new strategies (per your choice), tests each leak-free out-of-sample through E04, and **rejects** what doesn't generalize, with a complexity penalty. SHAP turns winners into human-readable rules. Full audit trail. Human-approves first, auto-promotes later.
  ...
  > **Een eerste, handmatige variant bestaat al** — de bottom-up "gemene-deler"-methode op de yes-marks: regime-clustering + parent-cascade + gestapelde vorm-subregels, met tijd-holdout, cross-coin en de sell-engine als eindtoets. Methode + status: [docs/methodology/rule-discovery.md](methodology/rule-discovery.md), skill `brain-rule-discovery`. **DIT IS DE KERN van nobrainersbot — de rules ZIJN het product; documentatie/vastlegging heeft hoogste prioriteit.** **Doel:** ~30-50% van de promising trades vangen (niet 100%) met per rule **20-23-kwaliteit** (selectiviteit ≤~0,1%, weinig slecht, gem ≥+0,7%/trade) en de eis dat een rule **over alle munten** voldoet; gemeten op gerealiseerde `profit_loss`, NOOIT best_upside; pre-registered permutatie-significant. **Mentaal model:** elke rule = smalle plak (20-23 hebben 39-81 subregels elk) → veel subregels per rule + meer rules. **Volgende bouwstap (rule 30):** regime/parent + een CHILD-rule die vanaf t2 álle ~30 berekeningen × lookback 1-20 doet (volume/prijs/vorm) om de slechte trades weg te filteren — van firehose naar 20-23-schaal. Open paden: eenzijdige drempels, fijner rooster, temporele subregels (t0→t1→t2). **Eindstand juni 2026 (geverifieerd, permutatie-getoetst):** op **2 munten** is geen 20-23-grade rule te vinden — enige reële signaal = NOS oversold + volume-magnitude (p=0,001, maar bescheiden + coin-specifiek); méér berekeningen → overfit, geen precisie. **De bindende beperking is het 2-munten-plafond; de enige echte hefboom is MÉÉR MUNTEN** (E07 nieuwe-coin-intake → E05 gating). De discovery-methode + harness + 20-23-lat staan klaar voor zodra er meer munten zijn. → **prioriteer E07/meer munten boven verder rule-zoeken op 2 munten.**
