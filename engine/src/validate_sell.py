...
def lock_floor(profit, minutes, buy, market, sl):
    ...
    return buy * sl["min_sl1"]          # fallback (ratchet inert)


def determine_stop(buy, market, minutes, profit, stop_prev, sl, i, buy_dt, max_price):
    ...
    lock = lock_floor(profit, minutes, buy, market, sl)
...
def replay(buy_dt, buy, sl):
    ...
    while i < len(DT) and (DT[i] - buy_dt).total_seconds() <= MAX_MIN * 60:
        ...
        stop, selling_price, orderstatus = determine_stop(buy, market, minutes, profit, stop_prev, sl, i, buy_dt, max_price)
...
for t in trades:
    ...
    if opl is not None:
        oracle_total_pl += opl
    if not near(r["profit_loss"], t["profit_loss"], 0.5) and len(examples) < 6:
        examples.append((t["ID"], t["profit_loss"], r["profit_loss"], str(t["selling_date"]), str(r["selling_date"])))
...
print("=" * 66)
if examples:
    print("profit_loss mismatches >0.5% (id, oracle_pl, my_pl, oracle_sell_dt, my_sell_dt):")
    for e in examples:
        print("  ", e)
src.close()
