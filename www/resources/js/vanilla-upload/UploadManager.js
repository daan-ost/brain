...
export class UploadManager {
    ...
    async loadBatchResultFile(batchId) {
        try {
            ...
            // Check if the result file is a ZIP - if so, extract it
            const isZip = data.file.name.toLowerCase().endsWith('.zip');

            if (isZip) {
                console.log('[UploadManager] Result file is a ZIP, extracting contents...', {
                    fileName: file.name,
                    fileSize: file.size,
                    pageSlug: this.config.pageSlug
                });

                // Extract ZIP contents via API (same as normal upload flow)
                await this.extractZipFiles([file]);

                console.log('[UploadManager] ZIP extraction complete, zipPreviews:', {
                    count: this.state.zipPreviews.length,
                    previews: this.state.zipPreviews.map(zp => ({
                        zip_name: zp.zip_name,
                        files_count: zp.files?.length || 0
                    }))
                });

                // Process ZIP file to get individual files
                const validFiles = [];
                await this.processZipFile(file, validFiles);

                console.log('[UploadManager] ZIP processing complete, validFiles:', {
                    count: validFiles.length,
                    files: validFiles.map(f => ({ name: f.name, isZipContainer: f.isZipContainer, isFromZip: f.isFromZip }))
                });

                // Check for non-container files (the actual extracted files)
                const nonContainerFiles = validFiles.filter(f => !f.isZipContainer);
                if (nonContainerFiles.length > 0) {
                    console.log('[UploadManager] Extracted', nonContainerFiles.length, 'files from ZIP');
                    ...
                    // Load preview for the first visible file
                    if (firstVisibleIndex !== -1) {
                        await this.loadPreview(firstVisibleIndex);
                    }

                    console.log('[UploadManager] Successfully loaded and extracted ZIP result from batch');
                    return true;
                } else {
                    console.warn('[UploadManager] ZIP extraction produced no valid files, falling back to single file view');
                }
            }

            // Non-ZIP file or ZIP extraction failed - load as single file
            const uploadedFile = {
    ...
    renderCoverPageSection() {
        ...
        // Version 2: NO TEMPLATE (expanded version with image)
        return `
            <div class="mt-6" data-test-id="cover-page-section">
                <div class="bg-gray-50 border-2 border-gray-200 rounded-lg p-6">
                    <div class="flex gap-6 items-start">
                        ...
                        <div class="flex-1 flex flex-col gap-4">
                            <!-- Title + Checkbox (Title first, checkbox right-aligned) -->
                            <div class="flex items-center justify-between">
                                ...
                                <input type="checkbox"
                                       ...
                                       ${disabledAttr}
                                       data-cover-toggle
                                       class="h-5 w-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 ${disabledClasses}">
                            </div>
    ...
    /**
     * Render "Continue to..." next step options after successful conversion
